...
### 2. Install the Language-specific Packages
...
1. If you don't have cmake or if your version of cmake is earlier than 3.6.1 (such as the default version of cmake on RHEL), run the following commands to install a newer version of cmake:
...
1. Create the locate database by running the following command:

	```bash
sudo launchctl load -w /System/Library/LaunchDaemons/com.apple.locate.plist
```
2. Locate the OpenMP library by running the following command:

	```bash
locate libgomp.dylib
```
3. To add OpenMP to your library path, replace ```path1``` in the following command with the output from the last command:

	```bash
ln -s path1 /usr/local/lib/libgomp.dylib
```

4. To build your changes, run the following command:
